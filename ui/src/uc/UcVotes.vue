...
import {voteUcApiClient} from "@/api";
...
const {
  ...
  queryFn: async () => {
    ...
    const { data } = await voteUcApiClient.vote.listMyVotes({
      page: page.value,
      size: size.value,
    ...
    onConfirm: async () => {
      try {
        const promises = selectedVoteNames.value.map((name) => {
          return voteUcApiClient.vote.deleteMyVote(
            {
              name: name
            }
          );
...
const handleDelete = async (vote?: Vote) => {
  Dialog.warning({
    ...
    onConfirm: async () => {
      try {
        await voteUcApiClient.vote.deleteMyVote({
        ...
        queryClient.invalidateQueries({ queryKey: ["uc:votes"] });
      }
    },
  });
};
